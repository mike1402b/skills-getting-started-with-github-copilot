...
def signup_for_activity(activity_name: str, email: str):
    ...
    # Validate email is from Mergington High School domain
    ...
    # Validate activity has not started yet (for simplicity, assume all activities are available)
    # (In a real application, you would check the current date/time against the activity schedule)
    # (For simplicity, we assume all activities are available and not started yet)
    if activity_name not in activities:
        raise HTTPException(status_code=404, detail="Activity not found")
